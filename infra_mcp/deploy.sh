...
echo -e "${CYAN}  (internal only — reachable from within VNet)${NC}"
...
echo -e "${YELLOW}========================================${NC}"
echo ""
echo -e "${CYAN}NOTE: This is a private MCP server (internal-only ingress).${NC}"
echo -e "${CYAN}Foundry must use Standard Agent Setup with private networking${NC}"
echo -e "${CYAN}on the same VNet (or peered) to reach this endpoint.${NC}"
echo ""
echo -e "${CYAN}Option A: Azure AI Foundry Portal${NC}"
echo ""
...
echo -e "  6. Authentication: Unauthenticated (network isolation)"
